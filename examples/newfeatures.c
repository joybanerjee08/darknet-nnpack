...
void write_detections(image im,int boxes, int num, float thresh, detection *dets, char **names, image **alphabet, int classes, char *save_name);
...
void apply_detector(network *net, char **names, char *outputs_folder_im, char *outputs_folder_txt, char *image_name, image im, image sized, image **alphabet, float thresh, float hier_thresh, char *show_img)
{
    ...
    if (outputs_folder_txt != 0){
       ...
       printf("\nWriting Detections: %s\n", finalTextFile);
       write_detections(im,nboxes, l.w*l.h*l.n, thresh, dets, names, alphabet, l.classes, finalTextFile);
       printf("\nDections saved: %s\n", finalTextFile);
    }
...
void write_detections(image im,int boxes, int num, float thresh, detection *dets, char **names, image **alphabet, int classes, char *save_name)
{
    ...
    //printf("\nFile Opened\n");
    for(i = 0; i < num && i < boxes; ++i){
      //printf("\nprob : %f",dets[i].prob[0]);
        int class = max_index(dets[i].prob, classes);
